...
- Clone the respository using `git clone https://github.com/simon-smart88/disagapp`
- Develop the new functionality outside of Shiny e.g. in an Rmarkdown file using example data.
...
- Install shinyscholar with `remotes::install_github("simon-smart88/shinyscholar")`
...
- Add checks at the start of the function that check that inputs are in the correct format and pass the errors to `writeLog` or `asyncLog` e.g.:
...
- Create a test file in the `tests/testthat/` directory called `test-fit_improved.R` and add tests that check the error handling functions correctly and that the function produced the desired results. 
You can find various data objects in `tests/testthat/helper_data.R` that may help to test the function. 
- Document the package using `devtools::document()` and then install with `devtools::install()`
- Navigate to the test file and click the "Run Tests" in the top right of the window pane.
- Once the tests are all passing you can begin to develop the Shiny module which consists of four files in the `inst/shiny/modules/` directory named with the module identifier and ending in `.yml`, `.R`, `.md` and `.Rmd`
...
- If the module produces a new type of object then you need to add a new object to the `common` data structure by editing the `common.R` file in the `inst/shiny/` directory. This is not necessary if, 
for example the module adds a new method for uploading response data or another covariate as these would add their results to `common$shape` and `common$covs` respectively.
...
- The `.md` file contains the module guidance. The background section should explain what the module does and what the scientific basis for it is. The implementation section should explain how the module works 
(e.g. which functions it uses) and how the user can use it i.e. what do they need to click.
