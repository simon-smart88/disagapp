incid_edit_module_ui <- function(id) {
  ns <- shiny::NS(id)
  tagList(
...
incid_edit_module_server <- function(id, common) {
  ...
  observeEvent(input$run, {
    ...
    # FUNCTION CALL ####
    shape <- incid_edit(common$shape, common$poly, input$type, common$logger)
...
incid_edit_module_map <- function(map, common) {
  map %>%
...
incid_edit_module_rmd <- function(common) {
  # Variables used in the module's Rmd code
  list(
    incid_edit_knit = !is.null(common$some_object),
