pred_pred_module_ui <- function(id) {
  ...
  tagList(
    ...
    input_task_button(ns("run"), "Produce predictions", type = "default", icon = icon("arrow-turn-down")),
    tags$br(),
    uiOutput(ns("dl_out"))
  )
}
...
pred_pred_module_server <- function(id, common, parent_session, map) {
  ...
  observeEvent(input$run, {
    ...
    if (input$uncertain){
      ...
      common$meta$pred_pred$uncertain_n <- 100
      common$meta$pred_pred$uncertain_ci <- 0.95
    }
