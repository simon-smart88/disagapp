...
transfer_covariates <- append(transfer_covariates, cov_bioclim(transfer_country, "{{meta$cov_bioclim$variables}}", transfer_shape))
```
...
```{r, echo = {{all(c(pred_transfer_knit, !is.null(meta$cov_nightlight$used)))}}, include = {{all(c(pred_transfer_knit, !is.null(meta$cov_nightlight$used)))}}}
  if (Sys.getenv("NASA_bearer") != ""){
    nasa_bearer = Sys.getenv("NASA_bearer")
  } else {
...
transfer_covariates <- append(transfer_covariates, cov_upload(transfer_covariate_file_df, transfer_shape))
```
